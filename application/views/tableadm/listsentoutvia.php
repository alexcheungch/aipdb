...
<div class="Hui-article">
    <article class="cl pd-20">
        <div class="text-right" style="margin-bottom: 20px;">
            <button type="button" onClick="javascript:window.location.href='<?php echo base_url('/TableAdm/createsentoutvia'); ?>'">+ Add</button>
        </div>
        <table id="coderesource_table" style="width: 100%;text-align: center;" class="table">
            ...
            <tbody>
            <?php           
            if($ListSentOutVia && count($ListSentOutVia)>0){
            foreach ($ListSentOutVia as $Record) {
            ?>
                <tr>
                	<td><?php echo $Record['ID'];?></td>
                    <td><?php echo $Record['SentOutMeans'];?></td>
                    <td>
